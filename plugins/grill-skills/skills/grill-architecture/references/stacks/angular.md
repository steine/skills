...
**Overlay — load *with* [ts.md](./ts.md), not instead of it.** Triggered when `@angular/core` is in deps. `ts.md` carries the universal layers (transport, domain, server/client state, DTO placement, shared-package, folders); this overlay carries Angular's **view-logic layer** and the per-lens tells `ts.md` defers.
...
Covered by `ts.md` (DTO/type home, shared-vs-feature, feature-leak-into-shared). No Angular-specific tell unless the slice introduces a shared `InjectionToken`/module that has leaked a domain concept. <!-- TODO: NgModule vs standalone boundary tells -->
