...
- `tsconfig.json` + **front-end** markers (`react`/`@angular/core`/`vue`, Vite/bundler, an `index.html`) → [references/stacks/ts-web.md](./references/stacks/ts-web.md) — language pack; **compose with a framework overlay**: `react` dep → [react.md](./references/stacks/react.md), `@angular/core` dep → [angular.md](./references/stacks/angular.md)
- `tsconfig.json` + **Node server** markers (`@nestjs/*`/`express`/`fastify`/`koa`, a server entry, an ORM like Prisma/TypeORM, **no DOM**) → [references/stacks/ts-node.md](./references/stacks/ts-node.md) — stub; backend layering, modelled on `dotnet.md`
...
**TypeScript hosts two stacks** — a browser SPA (`ts-web`) and a Node server (`ts-node`) — with different layering models. `tsconfig.json` alone is ambiguous: **disambiguate by dependencies** (the markers above). This is two stacks, not a tier flag on one — see MAINTAINERS.

Detect the stack during recon and **load the matching pack now** — packs compose, so a TS front-end loads `ts-web.md` *and* its framework overlay. Apply the per-lens tells as you run the lens below. A repo with both a backend and a front-end (or a full-stack TS monorepo — `ts-node` API package + `ts-web` app package) may need two packs for one grill — load each, per package. (grill-implementation keeps its own implementation-altitude packs separately, so each skill is self-contained.) If the stack isn't covered, use the .NET pack as the worked example, map by analogy, and flag the gap. If a lens doesn't bind (a deliberately flat codebase), say so per the pack rather than force-fitting.
